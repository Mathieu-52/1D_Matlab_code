...
switch choice
    case 1 
                k_rat=1;
             % same solubility in each phase 1 & 2
                D1 = 1.0;  
                D2 = 1.0; 
                D_s = 0.2;
                c1_t0 = 1.0;
            ...
            % Surfactant parameters
                ...
                r_a2=1;
                r_d1=1;
                r_d2=r_a2*(r_d1/r_a1)/k_rat; %require this for equilibrium 
                s_inf=1;
    ...
    case 2 
                k_rat=1;          
            % realistic solubility in one phase (large diff ratio)
            % D1 >> D2 -> adsorption ++ in phase 1 (and no in phase 2)
                D1 = 1.0;  
                D2 = 0.01;  
                D_s = 0.2;
                c1_t0 = 1.0;
    ...
    case 3 
                k_rat=3;
                %  confined to one phase and interface (choice phase 1)
                % no absorption to phase 2 and desorbtion from phase 1
                D1 = 1.0;  
                D2 = 0.0; 
                D_s = 0.2;
                c1_t0 = 3.0;
    ...
    case 5 
               k_rat=3;
             % limit of s_inf=0 (not on interface), including fake interface
                D1 = 1.0;  
                D2 = 1.0; 
                D_s = 0.2;
                c1_t0 = 1.0;
...
% identification de c1_tilde et c2_tilde 
% for any point in the mesh, we must have : c1(i) = c1_tilde*phi(i)
% and c1_tilde is a coefficient , then we can take a any point in the mesh
                    %c1_tilde = c1_np1(14)/phi_np1(14);
                    %c2_tilde = c2_np1(14)/(1-phi_np1(14));
...
% error 
                    ...
                    %fprintf('Step %d/%d: c1 max=%.3e, min=%.3e | c2 max=%.3e, min=%.3e | phi max=%.3e, min=%.3e\n', ...
                     %   idx, my_run.ndx, max_c1(idx), min_c1(idx), max_c2(idx), min_c2(idx), max_srf(idx), min_srf(idx));
...
mass_init  =  sum(c1_init + c2_init + srf_init);
mass_final = sum(c1_np1 + c2_np1 + srf_np1);
...
integral_ = integral(@(x) f_phi(x), -1, 1);
integral_2 = integral(@(x) f_phi(x).^2, -1, 1);
...
c2_ana_tilde = c2_analytical(r_a2,r_d2,c_total,s_inf,k_rat,integral_,integral_2); 
...
hold on;
plot(x_c, c2_ana, 'b-x', 'LineWidth', 2);
hold off;
...
if (choice ~= 5)
    ...
    figure;
    plot(x_c, srf  , 'r-x', 'LineWidth', 2);
    hold on;
    plot(x_c, S_ana, 'b-x', 'LineWidth', 2);
    hold off;
...
c1_ana = k_rat*c2_ana_tilde.*phi;

figure;
plot(x_c, c1_np1  , 'r-x', 'LineWidth', 2);
hold on;
plot(x_c, c1_ana, 'b-x', 'LineWidth', 2);
hold off;
